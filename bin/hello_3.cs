// Dafny program hello_3.dfy compiled into C#
...
// Dafny 3.2.0.30713
// Command Line Options: c:\Users\elija\Downloads\hello_3.dfy /verifyAllModules /compile:3 /spillTargetCode:1 /out:bin\hello_3
// hello_3.dfy
...
function method sumLeaves(t: Tree): int
  ensures t.Node? ==> sumLeaves(t) == sumLeaves(t.left) + sumLeaves(t.right)
  decreases t
{
  match t
  case Leaf(value) =>
...
function method allNodes(l: seq<Tree>): nat
  decreases l
{
  if l == [] then
    0
  else
    size(l[0]) + allNodes(l[1..])
}

function method allSum(l: seq<Tree>): int
  decreases l
{
  if l == [] then
    0
  else
    sumLeaves(l[0]) + allSum(l[1..])
}
...
method Main()
{
  var tree1 := myTree;
  var todo := [tree1];
  assert sumLeaves(todo[0].left) == allSum([todo[0].left]);
  assert sumLeaves(todo[0].right) == allSum([todo[0].right]);
  var a := allSum(todo[1..]) + sumLeaves(todo[0].left) + sumLeaves(todo[0].right);
  var b := allSum(todo[1..] + [todo[0].left] + [todo[0].right]);
  assert allSum(todo) == allSum(todo[1..]) + sumLeaves(todo[0].left) + sumLeaves(todo[0].right);
  assert allSum(todo) == allSum(todo[1..] + [todo[0].left] + [todo[0].right]);
  assert a == b;
  print a;
  print b;
}
...
namespace _module {
  ...
  public abstract class Tree {
    public Tree() { }
    private static readonly Tree theDefault = create_Leaf(BigInteger.Zero);
    public static Tree Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<Tree> _TYPE = new Dafny.TypeDescriptor<Tree>(Tree.Default());
    public static Dafny.TypeDescriptor<Tree> _TypeDescriptor() {
      return _TYPE;
    }
    public static Tree create_Leaf(BigInteger @value) {
      return new Tree_Leaf(@value);
    }
    public static Tree create_Node(Tree left, Tree right) {
      return new Tree_Node(left, right);
    }
    public bool is_Leaf { get { return this is Tree_Leaf; } }
    public bool is_Node { get { return this is Tree_Node; } }
    public BigInteger dtor_value {
      get {
        var d = this;
        return ((Tree_Leaf)d).@value; 
      }
    }
    public Tree dtor_left {
      get {
        var d = this;
        return ((Tree_Node)d).left; 
      }
    }
    public Tree dtor_right {
      get {
        var d = this;
        return ((Tree_Node)d).right; 
      }
    }
  }
  public class Tree_Leaf : Tree {
    public readonly BigInteger @value;
    public Tree_Leaf(BigInteger @value) {
      this.@value = @value;
    }
    public override bool Equals(object other) {
      var oth = other as Tree_Leaf;
      return oth != null && this.@value == oth.@value;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 0;
  ...
  public class Tree_Node : Tree {
    ...
    public Tree_Node(Tree left, Tree right) {
      this.left = left;
      this.right = right;
    }
    public override bool Equals(object other) {
      var oth = other as Tree_Node;
      return oth != null && object.Equals(this.left, oth.left) && object.Equals(this.right, oth.right);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 1;
  ...
  public partial class __default {
    public static BigInteger sumLeaves(Tree t) {
      Tree _source0 = t;
      if (_source0.is_Leaf) {
        BigInteger _73___mcc_h0 = ((Tree_Leaf)_source0).@value;
        BigInteger _74_value = _73___mcc_h0;
        return _74_value;
      } else {
        Tree _75___mcc_h1 = ((Tree_Node)_source0).left;
        Tree _76___mcc_h2 = ((Tree_Node)_source0).right;
        Tree _77_right = _76___mcc_h2;
        Tree _78_left = _75___mcc_h1;
        return (__default.sumLeaves(_78_left)) + (__default.sumLeaves(_77_right));
      }
    }
    public static BigInteger size(Tree t) {
      Tree _source1 = t;
      if (_source1.is_Leaf) {
        BigInteger _79___mcc_h0 = ((Tree_Leaf)_source1).@value;
        return BigInteger.One;
      } else {
        Tree _80___mcc_h1 = ((Tree_Node)_source1).left;
        Tree _81___mcc_h2 = ((Tree_Node)_source1).right;
        Tree _82_right = _81___mcc_h2;
        Tree _83_left = _80___mcc_h1;
        return ((BigInteger.One) + (__default.size(_83_left))) + (__default.size(_82_right));
      }
    }
    public static BigInteger allNodes(Dafny.ISequence<Tree> l) {
      BigInteger _84___accumulator = BigInteger.Zero;
    TAIL_CALL_START: ;
      if ((l).Equals((Dafny.Sequence<Tree>.FromElements()))) {
        return (BigInteger.Zero) + (_84___accumulator);
      } else {
        _84___accumulator = (_84___accumulator) + (__default.size((l).Select(BigInteger.Zero)));
        Dafny.ISequence<Tree> _in0 = (l).Drop(BigInteger.One);
        l = _in0;
        goto TAIL_CALL_START;
      }
    }
    public static BigInteger allSum(Dafny.ISequence<Tree> l) {
      BigInteger _85___accumulator = BigInteger.Zero;
    TAIL_CALL_START: ;
      if ((l).Equals((Dafny.Sequence<Tree>.FromElements()))) {
        return (BigInteger.Zero) + (_85___accumulator);
      } else {
        _85___accumulator = (_85___accumulator) + (__default.sumLeaves((l).Select(BigInteger.Zero)));
        Dafny.ISequence<Tree> _in1 = (l).Drop(BigInteger.One);
        l = _in1;
        goto TAIL_CALL_START;
      }
    }
    ...
    {
      Tree _86_tree1;
      _86_tree1 = __default.myTree;
      Dafny.ISequence<Tree> _87_todo;
      _87_todo = Dafny.Sequence<Tree>.FromElements(_86_tree1);
      BigInteger _88_a;
      _88_a = ((__default.allSum((_87_todo).Drop(BigInteger.One))) + (__default.sumLeaves(((_87_todo).Select(BigInteger.Zero)).dtor_left))) + (__default.sumLeaves(((_87_todo).Select(BigInteger.Zero)).dtor_right));
      BigInteger _89_b;
      _89_b = __default.allSum(Dafny.Sequence<Tree>.Concat(Dafny.Sequence<Tree>.Concat((_87_todo).Drop(BigInteger.One), Dafny.Sequence<Tree>.FromElements(((_87_todo).Select(BigInteger.Zero)).dtor_left)), Dafny.Sequence<Tree>.FromElements(((_87_todo).Select(BigInteger.Zero)).dtor_right)));
      Dafny.Helpers.Print(_88_a);
      Dafny.Helpers.Print(_89_b);
    }
    public static Tree myTree { get {
      return @Tree.create_Node(@Tree.create_Node(@Tree.create_Leaf(new BigInteger(20)), @Tree.create_Leaf(new BigInteger(3))), @Tree.create_Node(@Tree.create_Leaf(new BigInteger(7)), @Tree.create_Leaf(new BigInteger(-2))));
    } }
  }
