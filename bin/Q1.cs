...
// Dafny 3.2.0.30713
// Command Line Options: c:\Users\elija\Downloads\A3Files (1)\A3Files\Q1.dfy /verifyAllModules /compile:3 /spillTargetCode:1 /out:bin\Q1
// Q1.dfy

function sumOfInts(i: nat, n: nat): (r: nat)
  ensures r == i * (i + 2 * n - 1) / 2
  decreases i, n
{
  i * (i + 2 * n - 1) / 2
}
...
method triangular(n: nat, tri: Trisupport)
  ...
  ensures tri.col() == 0
  ensures tri.row() == n + 1
  ensures tri.printed() == old(tri.printed()) + sumOfInts(n, n)
  decreases n, tri
{
  var numOfCharsInLastRow: nat := (n + n - 1) * 2 + 1;
  var i: nat := 0;
  while i < n
    invariant tri.col() == 0
    invariant tri.row() == i + 1
    invariant 0 <= i <= n
    invariant i == 0 ==> tri.printed() == old(tri.printed())
    invariant i > 0 ==> tri.printed() == old(tri.printed()) + sumOfInts(i, n)
    decreases n - i
  {
    var numOfStars: nat := n + i;
    var numOfCharsForStarSpace: nat := (numOfStars - 1) * 2 + 1;
    var numOfSpacesOnEachSide: nat := (numOfCharsInLastRow - numOfCharsForStarSpace) / 2;
    assert tri.col() == 0;
    writeSideSpaces(numOfSpacesOnEachSide, tri);
    writeStarAreaRecur(numOfStars, tri);
    writeSideSpaces(numOfSpacesOnEachSide, tri);
    tri.cr();
    i := i + 1;
  }
...
method writeSideSpaces(numOfSpaces: nat, tri: Trisupport)
  ensures tri.row() == old(tri.row())
  ensures tri.col() == old(tri.col())
  ensures tri.printed() == old(tri.printed())
  decreases numOfSpaces, tri
{
  var i: nat := 0;
  while i < numOfSpaces
    decreases numOfSpaces - i
  {
    tri.space();
    i := i + 1;
  }
...
method writeStarAreaRecur(numOfStars: nat, tri: Trisupport)
  requires numOfStars > 0
  modifies tri
  ensures tri.row() == old(tri.row())
  ensures tri.col() == old(tri.col()) + numOfStars
  ensures tri.printed() == old(tri.printed()) + numOfStars
  decreases numOfStars
{
  tri.star();
  if numOfStars > 1 {
    tri.space();
    writeStarAreaRecur(numOfStars - 1, tri);
  }
}
...
class Trisupport {
  ...
  function method printed(): (r: nat)
    reads this`test_printed
    decreases {this}
  {
    test_printed
  }
...
namespace _module {
  ...
  public partial class __default {
    public static void triangular(BigInteger n, Trisupport tri)
    {
      BigInteger _49_numOfCharsInLastRow;
      _49_numOfCharsInLastRow = (((n) + ((n) - (BigInteger.One))) * (new BigInteger(2))) + (BigInteger.One);
      BigInteger _50_i;
      _50_i = BigInteger.Zero;
      while ((_50_i) < (n)) {
        BigInteger _51_numOfStars;
        _51_numOfStars = (n) + (_50_i);
        BigInteger _52_numOfCharsForStarSpace;
        _52_numOfCharsForStarSpace = (((_51_numOfStars) - (BigInteger.One)) * (new BigInteger(2))) + (BigInteger.One);
        BigInteger _53_numOfSpacesOnEachSide;
        _53_numOfSpacesOnEachSide = Dafny.Helpers.EuclideanDivision((_49_numOfCharsInLastRow) - (_52_numOfCharsForStarSpace), new BigInteger(2));
        __default.writeSideSpaces(_53_numOfSpacesOnEachSide, tri);
        __default.writeStarAreaRecur(_51_numOfStars, tri);
        __default.writeSideSpaces(_53_numOfSpacesOnEachSide, tri);
        (tri).cr();
        _50_i = (_50_i) + (BigInteger.One);
      }
    }
    public static void writeSideSpaces(BigInteger numOfSpaces, Trisupport tri)
    {
      BigInteger _54_i;
      _54_i = BigInteger.Zero;
      while ((_54_i) < (numOfSpaces)) {
        (tri).space();
        _54_i = (_54_i) + (BigInteger.One);
      }
    }
    public static void writeStarAreaRecur(BigInteger numOfStars, Trisupport tri)
    {
    TAIL_CALL_START: ;
      (tri).star();
      if ((numOfStars) > (BigInteger.One)) {
        (tri).space();
        BigInteger _in0 = (numOfStars) - (BigInteger.One);
        Trisupport _in1 = tri;
        numOfStars = _in0;
        tri = _in1;
    ...
    {
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("New\n"));
      Trisupport _55_tri;
      Trisupport _nw0 = new Trisupport();
      _nw0.__ctor();
      _55_tri = _nw0;
      __default.triangular(new BigInteger(10), _55_tri);
    }
  }

  public partial class Trisupport {
    public Trisupport() {
      this.test__printed = BigInteger.Zero;
      this.test__row = BigInteger.Zero;
      this.test__col = BigInteger.Zero;
    }
    public void __ctor()
    {
      (this).test__printed = BigInteger.Zero;
      (this).test__col = BigInteger.Zero;
      (this).test__row = BigInteger.One;
    }
    public BigInteger test__printed;public BigInteger test__row;public BigInteger test__col;public BigInteger printed() {
      return this.test__printed;
    }
    public void star()
    {
      (this).test__printed = (this.test__printed) + (BigInteger.One);
      (this).test__col = (this.test__col) + (BigInteger.One);
    ...
    public void cr()
    {
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("row="));
      Dafny.Helpers.Print(this.test__row);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString(" printed="));
      Dafny.Helpers.Print(this.test__printed);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
      (this).test__row = (this.test__row) + (BigInteger.One);
      (this).test__col = BigInteger.Zero;
    }
  }
